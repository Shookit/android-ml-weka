...
public class MultilayerPerceptron extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, Randomizable {
	...
	protected class NeuralEnd extends NeuralConnection {
		 ...
		 * 
		 * @param input
		 *            True if this unit is used for entering an attribute, False if it's used for determining a class value.
		 * @param val
	...
	/**
	 * Inner class used to draw the nodes onto.(uses the node lists!!) This will also handle the user input.
	 */
	...
	*//**
	 * This function gets called when the user has clicked something It will amend the current selection or connect the current selection to the new selection. Or if nothing was selected and the right button was used it will delete the node.
	 * 
	 * @param v
	...
	*//**
	 * A label to state roughly the accuracy of the network.(because the accuracy is calculated per epoch, but the network is changing throughout each epoch train).
	 */
	/*
	...
	*//**
	 ...
	 * 
	 * @param r
	 *            True if the network should restart with it's current options and set the learning rate to half what it currently is.
	 */
	...
	/**
	 * @param c
	 *            True if the class should be normalized (the class will only ever be normalized if it is numeric). (Normalization puts the range between -1 - 1).
	 */
	...
	/**
	 * @param a
	 *            True if the attributes should be normalized (even nominal attributes will get normalized here) (range goes between -1 - 1).
	 */
	...
	/**
	 * @return A string representing the hidden layers, each number is the number of nodes on a hidden layer.
	 */
	 ...
	 * @param inst
	 *            the instances.
	 * @return The modified instances. This needs to be done. If the attributes are normalized then deep copies will be made of all the instances which will need to be passed back out.
	 */
