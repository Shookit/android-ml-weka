 ...
 * Outputs the graph representation of the clusterer to the file. Only for clusterer that implemented the <code>weka.core.Drawable</code> interface.
 * <p/>
 * 
...
public class ClusterEvaluation implements Serializable, RevisionHandler {
	 ...
	 *            the set of instances to cluster
	 * @param testFileName
	 *            the name of the test file for incremental testing, if "" or null then not used
	 * 
	 * @throws Exception
	 *             if something goes wrong
	 */
	 ...
	 *            the name of the test file for incremental testing, if "" or null then not used
	 * @param outputModel
	 *            true if the clustering model is to be output as well as the stats
	 * 
	 * @throws Exception
	 *             if something goes wrong
	 */
	public void evaluateClusterer(Instances test, String testFileName, boolean outputModel) throws Exception {
		...
		for (i = 0; i < cc; i++) {
			if (instanceStats[i] > 0)
				m_clusteringResults.append(Utils.doubleToString((double) i, clustFieldWidth, 0) + "      " + Utils.doubleToString(instanceStats[i], numInstFieldWidth, 0) + " (" + Utils.doubleToString((instanceStats[i] / sum * 100.0), 3, 0) + "%)\n");
		}
	...
	/**
	 * Evaluates cluster assignments with respect to actual class labels. Assumes that m_Clusterer has been trained and tested on inst (minus the class).
	 * 
	 * @param inst
